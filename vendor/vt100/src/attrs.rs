...
pub struct Attrs {
    ...
    pub bgcolor: Color,
    pub mode: u8,
    /// BAUDE FORK (OSC 8): index into the `Screen`'s interned link table, or
    /// `None` when the cell is not part of a hyperlink. Riding on `Attrs`
    /// means scrollback/wrap/resize/overwrite/erasure inherit correctness
    /// from the existing grid model (LINK-03).
    pub link: Option<u16>,
}
