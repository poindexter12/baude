...
<!-- Current milestone: v0.7 Native Claude integration. See REQUIREMENTS.md. -->

- [ ] Capture the full Claude Code status-line payload (model, effort, thinking, PR, worktree), not just cost/context/rate-limits
- [ ] Drive working/waiting/done state from Claude Code hooks instead of the PTY-output-silence heuristic
- [ ] Surface a live per-session tool-activity timeline in the TUI and PWA
- [ ] Approve or deny a session's pending permission prompt from the PWA
...
|----------|-----------|---------|
| GSD-track baude starting at v0.7 (lean scaffold, no full re-interview) | Codebase is mature and well-understood; a full new-project interview would re-derive known facts | — Pending |
| Prefer first-party Claude data (status-line JSON, hooks) over inference | Accuracy + unlocks PR/effort/activity data for free | — Pending |
| Local hook transport via per-session event files; HTTP only in the daemon | Matches existing `meta.rs`/bridge file-tail patterns; avoids a new bind for the TUI | — Pending |
| Permission-prompt mode is opt-in; `skip` stays default | Unattended overnight runs must not block on phone approval | — Pending |
...
---
*Last updated: 2026-06-15 after lean GSD scaffold for milestone v0.7*
