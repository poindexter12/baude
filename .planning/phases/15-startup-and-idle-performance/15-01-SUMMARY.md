...
Orchestrator post-wave notes (2026-09-21): three executor rounds were needed. Round 1 delivered the dirty flag, first-frame flag, and PTY generation counter but left `app.restore()` before the loop, skipped the `step()` refactor and Task 2, and wrote tests that toggled fields. Round 2 added `step()`, deferred restore, remote `same_view()`, and the timing structs but never wired stages into startup. Round 3 wired the startup stages, `BAUDE_TIMING`/`--timing`, stderr output after terminal restore, and the waiting-row 1 Hz refresh, but kept the hollow idle-draw test "as adequate". The orchestrator's fix (15d48d7) made `step()` backend-generic (`step_with`), set `Stepped.restore_finished`, added `LoopTiming` for the `first_frame`/`session_restore`/`first_metadata_poll` stages shared by `run()` and tests, made the first tick poll metadata, and rewrote the three hollow tests to drive the real loop. Final orchestrator-run gates: fmt 0, clippy 0, build 0, `cargo test --workspace` 0 with 730 passed / 0 failed; both plan verify commands pass (8/8, 5/5). Rounds 2-3 and the fix landed without preceding `test(15-01)` commits (TDD-gate debt; round 1 has the test/feat pair).
